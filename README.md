...
フォールバック経路での翻訳は、`translators` に設定された順に処理される。現在の標準設定は DeepL → Google Cloud Translation。
...
`config.json` の `translators` に列挙した順に試行される。現在の標準設定は `["deepl", "google"]`。**未登録のバックエンドは自動スキップされる。**
