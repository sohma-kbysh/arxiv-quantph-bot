...
- On the Gemini path, the prompt instructs the LLM to return multiple IDs only when the paper genuinely spans multiple genres; duplicate genre IDs returned by the model are deduplicated while preserving order
...
- Classification stats: Gemini mode and success count, TF-IDF fallback count
...
- Gemini 経路では、LLM が複数ジャンルにまたがると判断した場合のみ複数 ID を返すよう指示している。モデルが同じ ID を重複して返した場合は、順序を保ったまま重複排除される
...
- 分類の内訳: Gemini のモードと成功数、TF-IDF フォールバック数
