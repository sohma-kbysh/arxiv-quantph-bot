...
- Papers whose pre-screen genres touch none of `prescreen_defer_genres` (default: `nisq`, `hardware`, `sensing`, `foundations`, `other`) form the **priority group** and are always classified with the primary model `gemini_model_primary` (`gemini-2.5-pro`)
- The rest form the **deferred group**. They are also classified with the primary model as long as the estimated number of Gemini requests for the run (priority batches + deferred batches) fits `gemini_primary_run_budget` (default: 60); otherwise the deferred group uses the secondary model `gemini_model_secondary` (`gemini-2.5-flash`) to stay inside the free-tier daily quota
- Each model has its own circuit breaker: persistent 429s or repeated 500/503s mark only that model dead for the rest of the run, and any papers still pending automatically fall through to the other model. Only when Gemini is entirely unavailable (no working model, or no API key) does the bot post the TF-IDF pre-screen result directly (emergency fallback, as before)
- `gemini_min_intervals` paces `gemini-2.5-pro` at 13s and `gemini-2.5-flash` at 7s between requests (the code enforces at least 13s for any model name containing "pro"); see "Notes" below for the underlying free-tier RPD/RPM limits
...
`classifier` records which model produced the classification: `"gemini-2.5-pro"`, `"gemini-2.5-flash"`, or `"tfidf"` (emergency fallback).
...
- Classification stats: per-model paper counts, e.g. `🏷 分類: gemini-2.5-pro: 13件 / gemini-2.5-flash: 21件 / TF-IDF: 2件`
...
| `gemini_model_primary` | `"gemini-2.5-pro"` | Primary classification model |
| `gemini_model_secondary` | `"gemini-2.5-flash"` | Used for the deferred group when the estimated request count exceeds `gemini_primary_run_budget`, and as a fallback when the primary model is rate-limited out |
| `gemini_min_intervals` | pro: `13`, flash: `7` | Per-model request spacing in seconds, overriding `gemini_min_interval_sec` |
| `gemini_primary_run_budget` | `60` | Estimated-request threshold above which the deferred group switches to the secondary model |
...
```text
[info] Gemini usage: mode=classify-only, model=gemini-2.5-pro+gemini-2.5-flash, requests=17, classified=82/82, tfidf_fallback=0, disabled_models=None
```
...
- Gemini free-tier RPD (requests per day) limits may change, so check the current values in [Google AI Studio](https://aistudio.google.com/) when setting it up. As of writing, the free tier allows `gemini-2.5-pro` 100 requests/day and 5 requests/minute (hence `gemini_min_intervals` paces it at 13s, and the code enforces at least 13s for any model name containing "pro") and `gemini-2.5-flash` 250 requests/day and 10 requests/minute; even the busiest day observed so far (~170 papers, in batches of 5) needed only ~34 requests, well inside both quotas.
...
現在の標準運用では、Gemini は**分類のみ**に使い、翻訳は DeepL → Azure Translator → Google Cloud Translation の順に試行する。Gemini の出力はジャンル ID だけなので、翻訳まで Gemini に任せる構成より API 消費を抑えやすい。分類自体は TF-IDF による事前分類で2つの Gemini モデルへ振り分けられ、多くの論文はプライマリモデル `gemini-2.5-pro` で、予算やレート制限の圧迫時は残りがセカンダリモデル `gemini-2.5-flash` で分類される。**標準ライブラリのみで動作し、`pip install` は不要。**
...
- 事前分類のジャンルが `prescreen_defer_genres`(デフォルト: `nisq`, `hardware`, `sensing`, `foundations`, `other`)のいずれにも触れない論文は**優先グループ**となり、常にプライマリモデル `gemini_model_primary`(`gemini-2.5-pro`)で分類する
- 残りは**繰り延べグループ**。この実行での推定リクエスト数(優先グループのバッチ数+繰り延べグループのバッチ数)が `gemini_primary_run_budget`(デフォルト60)以内であれば、繰り延べグループもプライマリモデルで分類する。超える場合はセカンダリモデル `gemini_model_secondary`(`gemini-2.5-flash`)を使い、無料枠の日次クォータ内に収める
- モデルごとに独立した circuit breaker を持つ: 持続的な 429、または 500/503 の連続発生は、そのモデルのみをこの実行で停止させる。未分類の論文は自動的にもう一方のモデルへフォールスルーする。Gemini が完全に利用不可(有効なモデルなし、または API キー未設定)の場合のみ、TF-IDF 事前分類の結果をそのまま投稿する(従来通りの緊急フォールバック)
- `gemini_min_intervals` により `gemini-2.5-pro` は13秒間隔、`gemini-2.5-flash` は7秒間隔でペーシングされる(コード側でもモデル名に "pro" を含む場合は最低13秒を強制する)。無料枠の RPD/RPM の詳細は後述の「留意事項」を参照
...
`classifier` はどのモデルが分類したかを記録する: `"gemini-2.5-pro"`、`"gemini-2.5-flash"`、または `"tfidf"`(緊急フォールバック)のいずれか。
...
- 分類の内訳: モデル別の投稿論文数(例: `🏷 分類: gemini-2.5-pro: 13件 / gemini-2.5-flash: 21件 / TF-IDF: 2件`)
...
| `gemini_model_primary` | `"gemini-2.5-pro"` | 分類のプライマリモデル |
| `gemini_model_secondary` | `"gemini-2.5-flash"` | 推定リクエスト数が `gemini_primary_run_budget` を超えた場合に繰り延べグループで使用。プライマリモデルがレート制限で停止した際のフォールバック先にもなる |
| `gemini_min_intervals` | pro: `13`, flash: `7` | モデルごとのリクエスト間隔(秒)。`gemini_min_interval_sec` を上書きする |
| `gemini_primary_run_budget` | `60` | この推定リクエスト数を超えると繰り延べグループがセカンダリモデルに切り替わる閾値 |
...
- Gemini 無料枠の RPD(1日あたりリクエスト数)制限は変更される可能性があるため、導入時に [Google AI Studio](https://aistudio.google.com/) で現行値を確認すること。執筆時点では無料枠は `gemini-2.5-pro` が100リクエスト/日・5リクエスト/分(そのため `gemini_min_intervals` で13秒間隔にペーシングしており、コード側でもモデル名に "pro" を含む場合は最低13秒を強制する)、`gemini-2.5-flash` が250リクエスト/日・10リクエスト/分。これまでで最も多かった日(約170論文、バッチサイズ5)でも約34リクエストで済み、両方の枠に十分収まる。
